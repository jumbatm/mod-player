...
class NoteMixer
{
    ...
    size_t m_jumpPosition =
        0;  // Where to jump to when indexing to end of sample?
...
public:
    NoteMixer(const Sample &s, double scaleFactor = 1.0, size_t startOffset = 0)
        : m_sample(s),
          ...
          m_jumpPosition(m_sample.repeatOffset /
          ...
          m_index(startOffset)
    {
        if (NoteMixer::playRate == 0)
        {
            throw std::runtime_error(
                "No player rate set for NoteMixer! Did you forget to set "
                "NoteMixer::playRate");
        }
    }
    ...
    uint8_t at(size_t index)
        const;  // TODO: Consider templating this for the return type.

    // Get the sample at the internal index (m_index) and increment it.
    uint8_t next();

    // Query the size after scaling. This is The size can't get larger, so
    // disallow
    // divisions less than 1.0.
    size_t size() const
    {
        return ::ceil(m_sample.sampleData.size() /
                      (m_scaleFactor > 1.0 ? m_scaleFactor : 1.0));
    }
