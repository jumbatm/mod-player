...
Sample::Sample(const std::vector<char> &sampleBlock)
{
    ...
    if (sampleBlock.size() != 30)
    {
        throw std::runtime_error(
            "Sample::Sample: Sample block not of correct size!");
    }

    // Use another iterator here to keep track of where we are in the sample
    // block.
    ...
    length = 2 * wordreader::readword<decltype(length)>(*cit, *(cit + 1),
                                                        wordreader::BIG);
    ...
    repeatOffset = 2 * wordreader::readword<decltype(repeatOffset)>(
                           *cit, *(cit + 1), wordreader::BIG);

    cit += 2;

    // Sample's repeat length in 2-byte words.
    repeatLength = 2 * wordreader::readword<decltype(repeatLength)>(
                           *cit, *(cit + 1), wordreader::BIG);
