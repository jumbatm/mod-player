...
{
    // Use an iterator so that we don't have to continually keep track of where
    // we are in the song data.
    std::vector<uint8_t>::const_iterator cit = songData.begin();

    // Read the title of the song.
    m_name = std::string(cit, cit + 20);

    cit += 20;  // Can't put this compound assignment in function call, as
                // functions calls are evaluated right to left.

    {  // Skip ahead and see how many instruments we have.
        // There can either be 15 or 31 instruments, so that's either at 1080 or
        // 600 respectively.
    ...
    // Read pattern table. (128 bytes). We only consider the first
    // m_numPatternsPlayed bytes.
    m_patternTable = std::vector<uint8_t>(cit, cit + m_numPatternsPlayed);

    // The largest value in the pattern table is equal to the number of patterns
    // - 1.
    ...
    size_t patternDataLength =
        (m_numChannels * /* 4 Bytes per note:*/ sizeof(uint32_t) *
         /* Number of lines per pattern:*/ 64 * m_numPatterns);
    ...
    for (auto &sample : m_samples)
    {
        ...
        sample.sampleData =
            std::vector<uint8_t>(songData.begin() + offset,
                                 songData.begin() + offset + sample.length);
        cit += sample.length;  // Take advantage of information we already have.
    }

    SOFT_ASSERT((std::cout << std::distance(cit, songData.cend()),
                 cit == songData.end()));
}
...
#define NAME_AND_VALUE(VAR) #VAR << ": " << VAR << "[END]\n"
    std::cout << NAME_AND_VALUE(m_name) << NAME_AND_VALUE(m_numChannels)
